...
class Wall_Func():
    def __init__(self, args) -> None:
        ...
        self.src_to_rx = 0.03
        # Geometry load
        self.base = os.getcwd() + '/Geometry_4W/Base'
        self.basefile = self.base + '/base{}.png'.format(i)
        self.geofolder = os.getcwd() + '/Geometry_4W/Object'
        self.geofile = self.geofolder + '/geometry{}.png'.format(i)

        # Data load
        self.pix =int(self.square_size/0.005)
        if not os.path.exists('./Input_4W'):
            os.makedirs('./Input_4W')        
        if not os.path.exists('./Input_4W/Base'):
            os.makedirs('./Input_4W/Base')
        if not os.path.exists('./Input_4W/Object'):
            os.makedirs('./Input_4W/Object')
        if not os.path.exists('./Output_4W'):
            os.makedirs('./Output_4W')
        if not os.path.exists('./Output_4W/Base'):
        ...
        if not os.path.exists('./ObjImg_4W'):
            os.makedirs('./ObjImg_4W')
        if not os.path.exists('./WallObj_4W'):
            os.makedirs('./WallObj_4W')

  
    def view_geometry(self):
        # self.preprocess(self.basefile)
        with h5py.File('./Geometry_4W/geometry_2d.h5', 'r') as f:
            data = f['data'][:]
    ...
    def preprocess(self, filename):
        ...
        with h5py.File('./Geometry_4W/' + self.filename, 'w') as file:
    ...
    def run_base(self):
        ...
        # Run gprMax
        self.input = './Input_4W/Base{}.in'.format(self.i)
        self.resol = 0.005
...
#geometry_objects_read: {pml + src_to_pml + 0.1:.3f} {self.square_size/2:.3f} {0:.3f} ./Geometry_4W/geometry_2d.h5 Base_materials.txt
...
geometry_view: 0 0 0 {domain_2d[0]:.3f} {domain_2d[1]:.3f} {domain_2d[2]:.3f} 0.005 0.005 0.005 Base n
        ...
        try:
            ...
            os.rename(output_file, f'./Output_4W/Base/Base{self.i}.out')
            plt.savefig(f'./BaseImg_4W/Base{self.i}' + ".png")
        except Exception as e:
            print(e)
    ...
    def run_2D(self):
        ...
        # Run gprMax
        self.input = './Input_4W/Object{}.in'.format(self.i)
        pml_cells = 20
        pml = self.resol * pml_cells
        src_to_pml = 0.04
...
#geometry_objects_read: {pml + src_to_pml + 0.1:.3f} {self.square_size/2:.3f} {0:.3f} Geometry_4W/geometry_2d.h5 Obj_materials.txt
...
geometry_view: 0 0 0 {domain_2d[0]:.3f} {domain_2d[1]:.3f} {domain_2d[2]:.3f} 0.005 0.005 0.005 Object n
        ...
        try:
            ...
            with h5py.File(f'./Output_4W/Base/Base{self.i}.out', 'r') as f1:
            ...
            ax.margins(0, 0)  # Remove any extra margins or padding
            fig.tight_layout(pad=0)  # Remove any extra padding
            plt.savefig(f'./WallObj_4W/Wall_Obj{self.i}' + ".png")
            ...
            os.rename(output_file, f'./Output_4W/Object/Obj{self.i}.out')
            plt.savefig(f'./ObjImg_4W/Obj{self.i}' + ".png")
        except Exception as e:
            print(e)
